...
아래 파일은 `src/App.tsx`에서 직접 참조하지 않아 GitHub 업로드 대상에서 제외했다. 실제 삭제하지 않고 `local_unused_data/public_data_unused/`로 이동했다.

| 파일명 | 처리 |
| --- | --- |
| `boundaries_analysis_v2.geojson` | local only |
...
| `final_building_v11_summary_v14.json` | local only |
| `final_full_planning_landuse_comparison_v24.json` | local only |
| `final_full_planning_landuse_summary_v24.json` | local only |
| `final_landuse_scope_comparison_v16.json` | local only |
| `final_landuse_scope_summary_v16.json` | local only |
